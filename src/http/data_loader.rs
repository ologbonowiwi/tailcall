...
where
  C: HttpClient + Send + Sync + 'static + ?Sized,
{
  pub client: Arc<C>,
  pub batched: Option<GroupBy>,
}
impl<C: HttpClient + Send + Sync + 'static + ?Sized> HttpDataLoader<C> {
  pub fn new(client: Arc<C>, batched: Option<GroupBy>) -> Self {
...
impl<C: HttpClient + Send + Sync + 'static + ?Sized> Loader<DataLoaderRequest> for HttpDataLoader<C> {
  type Value = Response;
